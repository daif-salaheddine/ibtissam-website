      ...
      >
        {doubled.map((item, i) => (
          <span key={i} className="inline-flex items-baseline gap-3 mx-8">
            <span className="font-cormorant text-xl md:text-2xl font-light" style={{ color: '#221F1C' }}>
              {item.name}
            </span>
            <span className="font-mono text-[0.5rem] tracking-[0.2em] uppercase" style={{ color: '#8A5C0A' }}>
              {item.category}
            </span>
            <span style={{ color: '#9B968F', marginLeft: 24 }}>·</span>
          </span>
        ))}
      </div>
    </div>
